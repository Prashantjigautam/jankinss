...
<body class="hm">
	...
	<div>
		<form action="<%=ORSView.SUPPLIER_LIST_CTL%>" method="post">
			...
			<%
				HashMap map = (HashMap) request.getAttribute("SupplierList");
			%>
			...
			%>
			<center>
				<h1 class="text-primary font-weight-bold pt-3">Supplier List</h1>
			</center>
			...
			<div class="row">
				...
				<div class="col-sm-2">
					<input class="form-control" type="text" name="payTerm"
						maxlength="10" placeholder="Enter paymentTerm" class="p1"
						value="<%=ServletUtility.getParameter("payTerm", request)%>">
				</div>
				<div class="col-sm-2">
					<input type="text" id="adate" name="date" class="form-control"
						placeholder=" Enter Registration date" readonly="readonly"
				...
				<table class="table  table-info table-bordered table-hover">
					...
					<tbody>
						<tr>
							...
							<td><%=dto.getName()%></td>
							<td><%=dto.getPayTerm()%></td>
							<td><%=DataUtility.getDateString(dto.getDate())%></td>
							<td><%=dto.getCategory()%></td>

							<td align="center"><a href="SupplierCtl?id=<%=dto.getId()%>">Edit</a></td>
						</tr>
					</tbody>
					<%
						}
